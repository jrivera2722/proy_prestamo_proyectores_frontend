...
const guardarDocente = async (bodyDocente) => {
    const data = axios.post(`http://localhost:8081/API/v1.0/Prestamos/docentes`, bodyDocente).then(r => r.data);
    return data;
}

const buscarPorCedulaDocente = async (cedula) => {
    const data = await axios.get(`http://localhost:8081/API/v1.0/Prestamos/docentes/${cedula}`).then(r => r.data);
    return data;
}

const buscarPorNombreDocente = async (nombre) => {
    const data = await axios.get(`http://localhost:8081/API/v1.0/Prestamos/docentes/n/${nombre}`).then(r => r.data);
    return data;
}

const buscarCartasCompromisoDocente = async (cedula) => {
    const data = await axios.get(`http://localhost:8081/API/v1.0/Prestamos/docentes/${cedula}/cartasCompromiso`).then(r => r.data);
    return data;
}

const buscarTodosDocentes = async () => {
    const data = await axios.get(`http://localhost:8081/API/v1.0/Prestamos/docentes`).then(r => r.data);
    return data;
}

const actualizarDocente = async (bodyDocente, cedula) => {
    const data = await axios.put(`http://localhost:8081/API/v1.0/Prestamos/docentes/${cedula}`, bodyDocente).then(r => r.data);
    return data;
}